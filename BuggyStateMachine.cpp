//22 Feb - ESP v1.01- Buggy State Machine by CF, DG & DF// Ready for Tech Demo: Motor Control 
    ...
    if (state == 2) {                   
        ...
        lcd.printf("Second State");
            while (state == 2&& paused == 0) {
                ...
                RightMotor = RightMotor - Directionval;          // Needs to be fixed: It allows for a negative value (   
                LeftMotor = LeftMotor + Directionval; 
                
                Right.write(1-RightMotor);                             
                Left.write(1-LeftMotor);
                ...
                //Default state: Automated movement 
                //V1=
                //V2=
                //V3=
    if (state == 3) {                               // State for showing task 6 
        Enable = 1;
        
        lcd.cls();
        lcd.locate(0,0);
        lcd.printf("Third State:");                           
                            
            while (state == 3 && paused == 0) { 
            wait (1);      
                 t3.start();                   
                    while (t3.read() < 1){ 
